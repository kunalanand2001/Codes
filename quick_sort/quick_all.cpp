...
void quickSort_first(int Ar[],int start, int end)
{
    if (start < end)
    {
        int pi = partition_first(Ar,start, end);

        quickSort_first(Ar,start, pi - 1);
        quickSort_first(Ar,pi + 1, end);
    }
}
...
void quickSort_last(int Ar[],int start, int end)
{
    if (start < end)
    {
        int pi = partition_last(Ar,start, end);

        quickSort_last(Ar,start, pi - 1);
        quickSort_last(Ar,pi + 1, end);
    }
}
...
// 	std::cout << std::chrono::duration_cast<std::chrono::nanoseconds>(finish-start).count()<<endl;